...
:class:`~zgrobot.config.Config` 继承自 `dict` 。因此， 你可以像使用普通 dict 一样使用它 ::
...
.. note:: 如果你在初始化 ``ZgRoBot`` 时传入了 ``config`` 参数， ``ZgRoBot`` 会忽略除 ``logger`` 外其他所有的初始化参数。 如果你需要对 ``ZgRoBot`` 进行一些配置操作， 请修改 Config 。
